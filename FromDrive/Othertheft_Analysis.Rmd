...
Neighborhood: The Vancouver Police Department uses Statistics Canada’s definition of neighbourhoods within municipalities. Neighbourhoods within the City of Vancouver are based on the census tract (CT) concept within the census metropolitan area (CMA).
...
Model1 <- gam(Total_Crimes ~ te(Latitude, Longitude, bs = c("tp", "tp"), k = c(12, 12)) +
                                s(Month, k = 12, bs = "cc"),
              data = train_data, method = "REML", family = poisson)
...
```{r}
Model1 <- gam(Total_Crimes ~ te(Latitude,Longitude, bs=c("tp","tp"), k=c(12,12)) + s(Month, k=12),data = train_data, method = "REML", family ="poisson")
summary(Model1)
```
